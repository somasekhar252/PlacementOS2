...
<<<<<<< HEAD
      <div className="bg-white p-10 rounded-[3rem] border border-slate-200 shadow-sm space-y-8">
        <div className="flex items-center gap-4">
          <span className="text-[10px] font-black uppercase text-slate-400 tracking-[0.3em]">Redirection & Integration Map</span>
=======
      {/* Integration Map */}
      <div className="bg-white p-10 rounded-[3rem] border border-slate-200 shadow-sm space-y-8">
        <div className="flex items-center gap-4">
...
>>>>>>> 619c02e09be8e47f5eb8092a7aefaab1d7fe74fe
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {[
<<<<<<< HEAD
            { name: 'Resume Worded', purpose: 'Scoring', link: 'https://resumeworded.com', desc: 'FAANG level scoring.' },
            { name: 'Jobscan', purpose: 'ATS Match', link: 'https://jobscan.co', desc: 'Beat automated filters.' },
            { name: 'Novorésumé', purpose: 'Inspiration', link: 'https://novoresume.com', desc: 'Clean designs.' },
            { name: 'LinkedIn Audit', purpose: 'Profile Opt', link: 'https://linkedin.com', desc: 'Optimize footprint.' }
=======
            { 
              name: 'Resume Worded', 
...
<<<<<<< HEAD
  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 py-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
    {[
      { name: 'The Scholar', desc: 'Academic LaTeX-inspired style with centered header and clean dividers.', color: 'slate' },
      { name: 'FAANG Modern', desc: 'Minimalist, impact-focused for product companies.', color: 'blue' },
      { name: 'The Executive', desc: 'Classic, professional design with authoritative sidebar.', color: 'slate' },
      { name: 'Chronological', desc: 'Traditional narrative focusing on time-line progression.', color: 'slate' },
      { name: 'Hybrid', desc: 'Combines technical skills matrix with a clean timeline.', color: 'indigo' }
    ].map((t, i) => (
      <div key={i} className="bg-white p-8 rounded-[3rem] border border-slate-200 shadow-sm hover:shadow-2xl transition-all group overflow-hidden relative">
        <div className={`absolute top-0 left-0 w-full h-2 bg-${t.color === 'slate' ? 'slate-800' : t.color + '-600'}`} />
        <div className="h-48 bg-slate-50 rounded-2xl mb-6 flex items-center justify-center text-5xl opacity-40 grayscale group-hover:grayscale-0 transition-all">📄</div>
        <h4 className="text-xl font-black text-slate-900 mb-2">{t.name}</h4>
        <p className="text-[10px] text-slate-500 leading-relaxed mb-6 font-black uppercase tracking-widest">{t.desc}</p>
        <button 
          onClick={() => onSelect(t.name)}
          className={`w-full bg-${t.color === 'slate' ? 'slate-800' : t.color + '-600'} text-white py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:opacity-90 transition-all active:scale-95`}
=======
...
<<<<<<< HEAD
  ...
  const handleAudit = () => {
    ...
    setTimeout(() => {
      setIsAuditing(false);
      setAuditResult("✅ GitHub Audit Passed: High commit density detected.");
=======
...
<<<<<<< HEAD
      <div className="bg-white p-10 rounded-[3rem] border border-slate-200 shadow-sm space-y-6 max-w-2xl mx-auto">
        <div className="flex items-center gap-4 mb-4">
            <div className="text-3xl">🐙</div>
            <div>
              <h4 className="text-xl font-black text-slate-900">Portfolio AI Audit</h4>
              <p className="text-xs text-slate-500 font-medium">Verify your digital footprint for recruiters.</p>
            </div>
        </div>
        <input 
          type="text" 
          ...
          className="w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl outline-none focus:ring-2 focus:ring-blue-600 transition-all text-slate-950 font-bold" 
        />
        <button 
          onClick={handleAudit}
          disabled={!githubUrl || isAuditing}
          className="w-full bg-slate-900 text-white py-4 rounded-2xl font-black uppercase tracking-widest disabled:opacity-50 transition-all hover:bg-blue-600"
        >
          {isAuditing ? 'Analyzing...' : 'Run Audit'}
        </button>
        {auditResult && (
          <div className="p-6 bg-blue-50 text-blue-700 rounded-2xl text-xs font-bold">
            {auditResult}
          </div>
        )}
      </div>
