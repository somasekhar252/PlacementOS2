...
<<<<<<< HEAD
    ...
    const timeoutId = setTimeout(() => {
      setLoading(false);
      alert("Request is taking longer than expected. Please try again or check your connection.");
    }, 60000); // 60 second timeout for AI synthesis
...
<<<<<<< HEAD
      ...
      try {
        await Promise.race([
          api.post('/generate-briefing', { profile: finalizedData }),
          new Promise((_, reject) => setTimeout(() => reject(new Error('Briefing timeout')), 30000))
        ]);
      } catch (briefingErr) {
